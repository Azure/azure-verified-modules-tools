...
- Managed-file sources can place a subtree under `modules/_all/` or
  `examples/_all/` to copy it into every existing immediate child directory.
  The reserved `_all` segment is not written to consumer repositories, so
  shared files such as `_footer.md` reach every terraform-docs target.
