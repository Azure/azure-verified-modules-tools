...
**Conclusion**: the silent failure is most consistent with the hosted runner
killing the `pwsh` process outright (e.g. an OOM-kill/SIGKILL), which bypasses
PowerShell's exception handling entirely -- not a reproducible library defect.
No further action was taken to "fix" PR 6484 or its module; there is nothing
reproducible to fix there.
...
- `./build.ps1 pre-commit` (layout + lint + test + component):
  ...
  - lint: OK (0 errors; the only warnings are pre-existing, unrelated to
    this change)
  - unit test: **1633 passed, 0 failed, 9 skipped**
  - component: **903 passed, 0 failed, 1 skipped**
  - Overall: build succeeded, 0 errors, 58 warnings (pre-existing, unrelated).
