...
**Last updated**: 2026-05-27 (Pinned-asset feature complete — slice 2 of 2. `Resolve-AvmPinnedAsset` private helper materialises a single asset descriptor into the per-user cache at `<Cache>/assets/<name>/<sha256>/`. Pipeline mirrors `Install-AvmToolFromLock`: cache-hit short-circuit → cross-process lock → re-check → stage under `.staging/<uuid>/` → `Invoke-AvmHttp` (SHA-verified download) → `Expand-AvmToolArchive` → optional Path subdir verification → atomic Move-Item → `.meta.json` + `.verified` marker. 12 new unit tests (cold + warm + force + subdir + 7 failure modes + zip-extension acceptance), 314 passed / 0 failed / 7 skipped overall. Deliberate scope cuts: `Sha256` required (ref-only and `type=git` throw `AvmConfigurationException` with "not yet supported" message); these land in a separate slice along with `.gitignore` write-back. With the asset layer complete, `avm check policy` (`conftest`) is now the only remaining Phase 2 §2 engine that doesn't need the A/B/C tool-binary decision.)
...
2. **Engine implementation unblockers** (one slice each — **see the Phase 2 §2 supply-chain audit below: 3 of 4 tools blocked on an architectural decision**):
   ...
   - [ ] `avm check policy` — `conftest test` against APRL + AVMSEC bundles; needs `conftest` in `tools.lock.psd1` + pinned APRL / AVMSEC bundles. **Tool unblocked** (`open-policy-agent/conftest` ships full 6-platform releases at `v0.68.2`); still waiting on the pinned-asset feature to deliver the OPA bundles.
