...
**Last updated**: 2026-05-21 (Phase 1: Bicep docs walker — top-level per-parameter detail blocks landed)
...
| 1     | Bicep facade                | **Walker advancing** — `format`/`lint`/`test` engines wired; `docs` ARM-JSON walker now emits the Resource Types, Parameters (summary tables + top-level detail blocks), and Outputs sections (object/array/UDT recursion, Usage Examples, Cross-refs still TODO); heavier verbs (`avm new`, `avm transform`, policy, convention) not started |
