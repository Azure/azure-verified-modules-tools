...
**Last updated**: 2026-05-27 (`conftest 0.68.2` pinned in `tools.lock.psd1` — first lock entry combining `platformAliases` + `archives` maps together. Extended `scripts/Update-AvmToolsLock.ps1` with a `-Conftest` parameter + `Get-ConftestEntry` private function (mirrors `Get-TerraformDocsEntry`, parses the upstream `checksums.txt`). Added a `Context 'platformAliases + archives combined'` validator test. Gate: 315 passed / 0 failed / 7 skipped, lint clean. `avm check policy` engine wiring is now the only Phase 2 §2 implementation slice unblocked by both pre-reqs — the engine just needs to call `Resolve-AvmTool conftest` + `Resolve-AvmPinnedAsset` for the APRL + AVMSEC bundles.)
...
2. **Engine implementation unblockers** (one slice each — **see the Phase 2 §2 supply-chain audit below: 3 of 4 tools blocked on an architectural decision**):
   ...
   - [ ] `avm check policy` — `conftest test` against APRL + AVMSEC bundles; needs `conftest` in `tools.lock.psd1` + pinned APRL / AVMSEC bundles. **Tool unblocked and pinned in lock** (`conftest 0.68.2` landed 2026-05-27; pinned-asset feature shipped 2026-05-27); only the engine wiring remains.
