...
**Last updated**: 2026-05-26 (Phase 2 §1 part 2: Integration-tier stub-binary smoke landed — `Invoke-AvmPreCommit` and `Invoke-AvmPrCheck` proven end-to-end against launcher-resolved `terraform`/`tflint`/`terraform-docs` stubs; latent `Read-AvmToolsLock` default-path off-by-one fixed)
...
| 2     | Terraform facade            | **Active — composition smoke tests landed at both Unit and Integration tiers; engine unblockers (`transform`/`check policy`/`check convention` + `format` `avmfix` chaining) are the next slices** — `format`/`lint`/`test`/`docs` engines wired; `transform`/`check policy`/`check convention` are still configuration-exception stubs |
