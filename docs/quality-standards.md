...
Their initial snapshots came from `legacy Terraform governance repository`; that
...
| Asset cache    | `%LOCALAPPDATA%\Avm\Cache\assets\<name>\<sha256>\<sub-path>`       | `…\Cache\assets\avm-mapotf-configs-pre-commit\<64-hex>\mapotf-configs\common\order_resource_attrs.mptf.hcl` | **196** | **64**             |
...
> **Upstream snapshot reviewed:** legacy Terraform governance repository at commit `65182443`.
...
>
> **⚠️ Row #5 `git_ignore` superseded 2026-08-13 (managed-lines sync).** The `GitignoreMustContain` primitive and its `avm.tf.gitignore-essentials` rule have been **removed** from the convention framework. The governance repository's per-group `_config.json` now carries a `managedLines` spec that enforces the same `.gitignore` line set (a strict superset of the rule's 23 globs) through `avm sync`, which runs as the first `pre-commit` step and as a hard-fail drift check in `pr-check`. The audit's *conclusion* — that the glob set must be enforced and repaired — still stands; only the *mechanism* changed, from a convention rule to the managed-files line spec. The primitive count in the table below is therefore three, not four.
>
> **⚠️ Partially reversed 2026-07-08 (managed-files sync restored as a dedicated verb).** Rows **#6 `deprecated_files`** ("Drop (mostly)") and **#7 `managed_files`** ("Drop entirely") are superseded *for the file-sync concern only*: the tooling now ships an `avm sync` verb (`Engines/ManagedFiles/Sync-AvmManagedFile.ps1`) that re-introduces governance managed-file + deprecated-file syncing, modelled on the legacy Terraform governance repository's `tf-repo-mgmt/` layout (its `repository-config/config.json` repository-groups + `deprecated-files.json` + `managed-files/root` overlay), **file-sync only (no PR creation)**, wired as the FIRST `pre-commit` step and into `pr-check` (drift = hard fail). This does **not** re-open the `check convention` decisions: the audit's core stance — *no bulk byte-match primitive inside the convention framework* (§"Cross-cutting decisions" #1) — still holds, because managed-files sync is a **separate engine/verb**, not a convention rule. So the "wrong shape for `avm check convention`" reasoning in rows #6/#7 is unaffected; only the "drop the capability entirely" conclusion is reversed. See [`progress.md`](progress.md) Part 2.
...
| 3 | `ensure_file_existence.grept.hcl` | `terraform.tf` and `_header.md` must exist. Fix: writes empty content (`""`).                                                                                                                                                                                                                                                | No. Terraform tolerates `terraform { }` in any file; `terraform-docs` requires the configured `_header.md` path to exist.              | **PS rule**       | `FileMustExist` (+ optional content fix)   | `terraform.tf` is module-specific and remains strict-only. A missing `_header.md` is deterministic: pre-commit creates a single folder-derived heading so terraform-docs can run, while authors can replace it with richer prose.                                                                                                                                                                                                  |
| 4 | `ensure_dir_existence.grept.hcl`  | `examples/` and `tests/` directories must exist. Fix: creates `examples/.gitkeep` and `tests/.gitkeep`.                                                                                                                                                                                                                       | No.                                                                                                                                     | **PS rule**       | `DirectoryMustExist`                       | `examples/` must contain at least one immediate example subdirectory and remains strict-only because no useful example can be synthesized. A missing `tests/` is safely repaired with `tests/.gitkeep`, preserving the required location until tests are added.                                                                                                                                                                   |
| 5 | `git_ignore.grept.hcl`            | `.gitignore` must contain a specific set of 24 globs (state, plans, lockfiles, `.terraform/`, tfvars, `.DS_Store`, `crash.log`, `.avm`, etc.). Fix: appends missing entries via `git_ignore` fix block.                                                                                                                       | No. `.gitattributes` controls EOL/encoding, not what's tracked.                                                                         | ~~**PS rule**~~ **Managed lines** (2026-08-13) | ~~`GitignoreMustContain`~~ none              | High-value: the difference between accidentally committing 50 MB of `.terraform/providers/` (or worse, `secret.tfvars`) and not. Keep the assertion *and* the append-missing-globs fix — both are safe and idempotent. **Superseded 2026-08-13:** delivered by the managed-files `managedLines` spec rather than a convention rule; see the banner at the top of this appendix.                                                                                |
...
The two `FileMustNotExist` rename fixes reconcile collisions instead of failing:
if the plural destination exists, they append non-whitespace singular-file
content with a safe line boundary and then remove the singular file. A
whitespace-only singular file is removed without changing the destination.

### Slice C primitive set (the only Slice C deliverable that depends on this audit)

After dispositions above, Slice C needs to build exactly **four** primitives, not the seven the plan listed as illustrative (reduced to **three** on 2026-08-13 when `GitignoreMustContain` was retired — see the banner at the top of this appendix):

| Primitive             | Used by | Slice D rule modules (count)                                                                |
| --------------------- | ------- | ------------------------------------------------------------------------------------------- |
| `FileMustNotExist`    | #1, #2  | 2 (one per filename)                                                                        |
| `FileMustExist`       | #3      | 1 (parameterised with `terraform.tf` + `_header.md`; optional content-template fix)          |
| `DirectoryMustExist`  | #4      | 1 (parameterised with `examples/` + `tests/`; optional child count / placeholder-file fix)   |
| ~~`GitignoreMustContain`~~ | ~~#5~~ | **Removed 2026-08-13** — replaced by the managed-files `managedLines` spec                |
...
2. **Targeted `.gitkeep` materialisation only.** A missing `tests/` gets `tests/.gitkeep`; `examples/` never gets a placeholder because it must contain a real example subdirectory.
3. **No empty required-file fixes.** `terraform.tf` remains strict-only. `_header.md` gets a meaningful folder-derived heading rather than an empty file.
...
- A future opt-in `avm new -Ecosystem terraform` scaffolder that emits a known-good starting tree (real `terraform.tf`, descriptive `_header.md`, real `.gitignore`, real `examples/default/main.tf`). Replaces the *useful* half of `managed_files` (the "give me a starting point" half) without the SHA1-lock half.
- ~~Per-module overrides for the `GitignoreMustContain` glob set (mechanism above; default list ships in Slice D).~~ Moot from 2026-08-13 — the glob set now lives in the managed-files `managedLines` spec.
...
> **UPDATE 2026-09-19 - the AzAPI request-header cleanup is retired.** The
> September 2026 fleet migration removed the legacy headers, helper locals, and
> forwarding inputs from active modules. The temporary
> `remove_avm_headers_for_azapi` common-profile rule is no longer shipped or
> evaluated on every transform target. The historical audit below is retained
> to explain the previous behavior.
>
...
> 2. **mapotf gained ordering/sorting transform primitives.** The per-config audit below predates `reorder_attributes`, `sort_blocks_in_file`, `remove_block_element`, and `move_block`. The live rules are partitioned into `common`, `module`, and `root` profiles without duplicating config files. Together they realise the **entire** avmfix 10-behaviour catalogue from Appendix C — including the file-partitioning behaviours #5 + #7 that [Appendix I](#appendix-i-decision-hcl2json-adoption-for-narrow-file-layout-enforcement) was going to cover with `hcl2json`.
...
**Context.** Upstream `legacy Terraform governance repository at commit 65182443` invokes `mapotf transform --mptf-dir <pinned> --tf-dir .` (then `mapotf clean-backup --tf-dir .`) against three configs at `mapotf-configs/pre-commit/*.mptf.hcl`. This audit answers, per config: (1) what does it concretely do; (2) is it important to the AVM contract; (3) what is the cheapest replacement (native PowerShell / general CLI / keep upstream via build-and-host); (4) effort. Read it before Slice G (`Invoke-AvmTerraformTransform`).
...
2. **Configs evolve upstream.** New `azapi_*` resource types, telemetry payload churn, and provider-version drift all happen on `legacy Terraform governance repository`'s schedule. A verbatim-upstream pin gives us one SHA to bump; a rewrite forks us into perpetual catch-up.
...
> - **#3 azapi overrides** → `order_resource_attrs`.
...
2. **Pick a hosting strategy for avmfix release artefacts.** Same A/B/C as mapotf: (i) PR a release workflow into upstream `lonegunmanb/avmfix`; (ii) build + host artefacts in `legacy Terraform governance repository` releases; (iii) build + host in this repo's own releases. **User decision** — resolve once for both mapotf + avmfix.
...
- **One existing telemetry channel in the AVM contract — but it is *not* this one.** At the time, `main.telemetry.tf` emitted module-deployment telemetry through `modtm`. The current Terraform transport is an empty AzAPI deployment generated by MaPoTF (Appendix B's 2026-09-24 update). Both transports measure module deployments, not CLI invocations; CLI telemetry remains a separate design.
...
| --------------------------------- | -------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------- |
| **Module-deployment telemetry**   | AVM Terraform module contract          | Normal Terraform applies of an instrumented module | `resource.azapi_resource.telemetry` in `main.telemetry.tf` (materialised by `mapotf transform`) | `var.enable_telemetry = false` in the consumer's Terraform config                                          |
...
- `Avm.Authoring` MUST NOT read or write the module's `terraform_data.telemetry` ID or its legacy `random_uuid.telemetry` state.
- CLI telemetry settings MUST NOT influence `var.enable_telemetry` in any module under test. The separate MaPoTF profiles forward the module's opt-out to supported child and example calls; the example input defaults to `true`, independently of CLI telemetry settings.
- `Avm.Authoring`'s proposed CLI install-id and the module's instance ID are unrelated; they MUST NOT be derived from each other.
- Module telemetry uses an Azure ARM deployment; proposed CLI telemetry uses a separate Application Insights resource. Neither channel reuses the legacy `modtm` endpoint.
...
pre-commit pivot. The packaged MAPOTF configuration now lives under scoped directories beneath
`src/Avm.Authoring/Resources/mapotf/` and is authoritative in this
...
Three upstream facts changed since the 2026-05/2026-06 audits, reported by the user on 2026-06-19 and verified against the legacy Terraform governance repository and `Azure/mapotf`:
...
2. **`grept` is gone, replaced by "repo sync" in the governance repo.** Repo-level managed-file synchronisation moved to the legacy Terraform governance repository's `tf-repo-mgmt/` PowerShell tooling — a **repo-scaffolding** layer, not a per-module pre-commit step. It is **out of scope** for `avm pre-commit` (see J.6). The custom-instruction "grept → PowerShell modules" is satisfied *upstream* by `tf-repo-mgmt`; our per-module convention rules (Slices C/D, already shipped) cover the per-module subset that does belong in pre-commit.
...
2. `mapotf transform --mptf-dir "$AVM_MPTF_URL" --tf-dir .` — `AVM_MPTF_URL` formerly defaulted to the legacy governance-hosted MAPOTF bundle.
...
- **pre-commit FIXES** — deterministic convention fixes run first, then `mapotf transform` mutates files in place (leaving `.tf.mptfbackup`, removed by `clean-backup`). Rules without an associated fix are excluded.
...
2. **Supply the configs (decision A - RESOLVED = package).** The configs
   ship under scoped directories beneath `src/Avm.Authoring/Resources/mapotf/`.
...
- **(A) How to supply the configs - RESOLVED = package.** The original
  ...
  `src/Avm.Authoring/Resources/mapotf/`; the transitional upstream
