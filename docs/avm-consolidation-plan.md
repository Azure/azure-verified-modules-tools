...
| `avm pr-check`                | Composition (as wired today): `format` → `transform` → `lint` → `check policy` → `check convention` → `test` → `docs` (stubbed Bicep engines for `transform` / `check policy` / `check convention` report as `skipped`) | Composition (as wired today): same 7-step chain; `transform` / `check convention` report as `skipped` until the Phase 2 §2 supply-chain decision unblocks `mapotf` / `grept` |
...
| 4     | Replace `grept`; audit `mapotf` + `avmfix` | Medium               | `grept` policies all dispositioned in `quality-standards.md` § Appendix A (5 ported via 4 primitives, 2 dropped); `mapotf` audit complete in § Appendix B (keep upstream via build-and-host recommended); `avmfix` audit complete in § Appendix C (keep upstream via build-and-host recommended; same hosting question as mapotf) |
