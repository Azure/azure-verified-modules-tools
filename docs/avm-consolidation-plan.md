...
**Exit criteria**: every `grept` policy is dispositioned (done — see [`quality-standards.md` § Appendix A](quality-standards.md#appendix-a-decision-grept-policy-disposition)) with the kept ones ported to PowerShell rule modules; `mapotf` audit complete (done — see [§ Appendix B](quality-standards.md#appendix-b-decision-mapotf-replacement-strategy)); `avmfix` audit deliverable landed in `quality-standards.md` with an approved implementation path; the rule framework loads built-in, pinned-asset, and per-repo rules with precedence rules documented.
...
| 4     | Replace `grept`; audit `mapotf` + `avmfix` | Medium               | `grept` policies all dispositioned in `quality-standards.md` § Appendix A (5 ported via 4 primitives, 2 dropped); `mapotf` audit complete in § Appendix B (keep upstream via build-and-host recommended); `avmfix` audit deliverable landed in `quality-standards.md` |
