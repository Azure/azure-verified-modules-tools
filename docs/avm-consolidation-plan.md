...
- **`mapotf` was audited before implementation** (done — see [`quality-standards.md` § Appendix B](quality-standards.md#appendix-b-decision-mapotf-replacement-strategy)). The packaged nine-rule bundle preserves declaration ordering, telemetry resource generation, provider constraints, and file hygiene. As of 2026-09-02, `remove_avm_headers_for_azapi` removes the legacy AVM contribution from AzAPI lifecycle headers and nested-module tracing inputs while preserving unrelated custom headers. `main_telemetry_tf` retains only the `modtm_telemetry` resource path and removes its obsolete header helper locals. `Invoke-AvmTerraformTransform` wraps the pinned `Azure/mapotf` release with `mapotf transform` + `mapotf clean-backup`.
