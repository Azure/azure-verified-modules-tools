...
- **`mapotf` was audited before implementation** (done — see [`quality-standards.md` § Appendix B](quality-standards.md#appendix-b-decision-mapotf-replacement-strategy)). Rules are packaged under scoped profiles: `common` orders files everywhere; `module` maintains providers and layout; `root` generates metadata-backed AzAPI deployment telemetry for the repository root and instrumented children. `module-call` forwards telemetry opt-out and location to local children after they acquire their inputs; `example` forwards the same controls to supported example calls; `test` removes obsolete `modtm` provider requirements in standalone test modules. Standard empty test mocks and resource references are migrated alongside the rules. Legacy `modtm` and telemetry-only random UUID instances are forgotten through no-destroy `removed` blocks. The AzAPI request-header cleanup was completed and retired in September 2026. The optional `telemetry_location` input defaults to `null` when `var.location` exists (using it as the fallback), or to `westus2` otherwise. Telemetry is reported solely through a deployment name with a fixed seven-hex Terraform prefix, the full version, a one-character source type, and a stable four-character instance suffix; a changing empty-template output forces each normal apply to write without adding reporting tags.
