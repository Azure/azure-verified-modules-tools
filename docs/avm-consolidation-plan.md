...
- **`mapotf` was audited before implementation** (done — see [`quality-standards.md` § Appendix B](quality-standards.md#appendix-b-decision-mapotf-replacement-strategy)). The packaged eight-rule pre-commit bundle preserves declaration ordering, telemetry resource generation, provider constraints, and file hygiene. As of 2026-09-02, a separate cleanup bundle removes the legacy AVM contribution from AzAPI headers and module inputs while preserving unrelated custom headers. The engine applies the full bundle to the root, applies cleanup to the root and every local module root under `modules/` identified by its own `terraform.tf`, then runs `mapotf clean-backup`. `main_telemetry_tf` retains only the `modtm_telemetry` resource path.
