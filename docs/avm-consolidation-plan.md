...
- **`mapotf` was audited before implementation** (done — see [`quality-standards.md` § Appendix B](quality-standards.md#appendix-b-decision-mapotf-replacement-strategy)). Rules are packaged once under scoped profiles: `common` applies in-place ordering and retired-header cleanup everywhere; `module` adds provider and file-layout rules for the root and local submodules; `root` adds `modtm_telemetry` only at the repository root. Examples run `example` then `common`, disabling telemetry on calls whose module declares `enable_telemetry` while preserving a single-file `main.tf` layout.
