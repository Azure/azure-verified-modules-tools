...
- **`mapotf` was audited before implementation** (done — see [`quality-standards.md` § Appendix B](quality-standards.md#appendix-b-decision-mapotf-replacement-strategy)). Rules are packaged once under scoped profiles: `common` applies in-place ordering and retired-header cleanup everywhere; `module` adds provider and file-layout rules for the root and local submodules; `root` adds `modtm_telemetry` only at the repository root. Examples run `example` then `common`, passing `var.enable_telemetry` on supported calls. The example variable defaults to true, with existing declarations preserved in place and missing declarations added to `variables.tf`; other example blocks keep their authored layout.
