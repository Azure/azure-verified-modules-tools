...
- `avm` verbs route to the Terraform stack:
  ...
  - `lint` → cleaned temporary module copy, `terraform init -upgrade` per scope, optional example `tflint-pre.ps1`, then `tflint` with merged AVM config (resolver picks `avm.tflint_module.hcl` vs `avm.tflint_example.hcl`).
...
- **`grept` is replaced, not ported.** Convention checks become first-party PowerShell rule modules under `src/Avm.Authoring/Engines/Terraform/` plus a rule loader in `Private/Rules/`. Each of the 7 upstream `.grept.hcl` policies (`outputs_tf`, `variables_tf`, `ensure_file_existence`, `ensure_dir_existence`, `git_ignore`, `deprecated_files`, `managed_files`) was audited individually and dispositioned in [`quality-standards.md` § Appendix A](quality-standards.md#appendix-a-decision-grept-policy-disposition). Outcome: 5 policies become PowerShell rules built on 4 primitives (`FileMustNotExist`, `FileMustExist`, `DirectoryMustExist`, `GitignoreMustContain`); 2 policies (`deprecated_files`, `managed_files`) are dropped entirely — see appendix for rationale. **Updated 2026-08-13:** `GitignoreMustContain` and its `avm.tf.gitignore-essentials` rule were removed once the managed-files `managedLines` spec took over `.gitignore` enforcement, leaving 4 rules on 3 primitives. No `grept` binary in `avm.pins.jsonc`, no `grept-policies` pinned asset.
- **`mapotf` was audited before implementation** (done — see [`quality-standards.md` § Appendix B](quality-standards.md#appendix-b-decision-mapotf-replacement-strategy)). Rules are packaged under scoped profiles: `common` orders files everywhere; `module` maintains providers and layout and creates a missing required `var.location` for Azure-deploying scopes; `root` generates metadata-backed AzAPI deployment telemetry for the repository root and instrumented children. `module-call` forwards the opt-out and missing location inputs to local children from deepest to root while retaining authored per-item locations; `example` forwards the same controls to supported example calls; `test` removes obsolete `modtm` provider requirements in standalone test modules. Standard empty test mocks and resource references are migrated alongside the rules. Legacy `modtm` and telemetry-only random UUID instances are forgotten through no-destroy `removed` blocks. The AzAPI request-header cleanup was completed and retired in September 2026. `telemetry_location` is removed; every non-exempt root and Azure-resource child declares `var.location`, and telemetry uses that input without a silent regional fallback. Utility modules that deploy no Azure resources stay exempt. Telemetry is reported solely through a deployment name with a fixed seven-hex Terraform prefix, the full version, a one-character source type, and a stable four-character instance suffix; a changing empty-template output forces each normal apply to write without adding reporting tags.
...
**Exit criteria**: every `grept` policy is dispositioned (done — see [`quality-standards.md` § Appendix A](quality-standards.md#appendix-a-decision-grept-policy-disposition)) with the kept ones ported to PowerShell rule modules; `mapotf` audit complete (done — see [§ Appendix B](quality-standards.md#appendix-b-decision-mapotf-replacement-strategy)); `avmfix` audit complete (done — see [§ Appendix C](quality-standards.md#appendix-c-decision-avmfix-replacement-strategy)); the rule framework loads built-in, pinned-asset, and per-repo rules with precedence rules documented.
...
| 4     | Replace `grept`; audit `mapotf` + `avmfix` | Medium               | `grept` policies all dispositioned in `quality-standards.md` § Appendix A (5 ported via 4 primitives, 2 dropped; reduced to 4 rules on 3 primitives on 2026-08-13 when `GitignoreMustContain` was retired in favour of the managed-files `managedLines` spec); `mapotf` audit complete in § Appendix B (keep upstream via build-and-host recommended); `avmfix` audit complete in § Appendix C (keep upstream via build-and-host recommended; same hosting question as mapotf) |
