...
| `avm lint`            | `tflint`                | per root / direct `modules/*` / direct `examples/*`: `--init`, then `--config <scope-config> --format=json --minimum-failure-severity=warning` |   ✅   | Repository-root `avm.tflint*.override.hcl` files merge over the corresponding vendored config before any scope runs. Exit `2` means findings. |
