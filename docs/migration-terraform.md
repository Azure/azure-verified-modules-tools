...
| `avm transform`       | `mapotf`                | root `transform --mptf-dir <pre-commit-config> --tf-dir <root>`, cleanup `transform --mptf-dir <cleanup-config> --tf-dir <target>` for the root and every `modules/**/terraform.tf` module root, then `clean-backup --tf-dir <root>` |   ✅   | Uses pinned mapotf. The cleanup-only passes remove legacy AzAPI telemetry headers and helper locals from local submodules without generating submodule telemetry or modifying downloaded modules. Pr-check snapshots and restores source files while reporting transform drift. |
