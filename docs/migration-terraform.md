...
| `avm transform`       | `mapotf`                | root: `root,module,common`; each `modules/**/terraform.tf`: `module,common`; each direct `examples/*`: `common`; then `clean-backup` per target |   ✅   | Repeated `--mptf-dir` values compose scoped profiles. Root-only telemetry never reaches submodules or examples. Module file-layout rules never reach examples, so example blocks may remain together in `main.tf`. Pr-check snapshots and restores source files while reporting transform drift. |
