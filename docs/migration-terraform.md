...
| `avm lint`            | `tflint`                | per root / direct `modules/*` / direct `examples/*`: optional `tflint-pre.ps1` for examples, `--init`, then `--config <scope-config> --format=json --minimum-failure-severity=warning` |   ✅   | Repository-root `avm.tflint*.override.hcl` files merge over the corresponding vendored config before any scope runs. `tflint-pre.sh` is rejected with PowerShell migration guidance. Exit `2` means findings. |
