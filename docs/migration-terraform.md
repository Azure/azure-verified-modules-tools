...
| `avm transform`       | `mapotf`                | `transform --mptf-dir <vendored-config> --tf-dir <root>` then `clean-backup --tf-dir <root>`                                 |   ✅   | Uses pinned mapotf and vendored pre-commit configs. The bundle removes legacy AzAPI telemetry headers and their helper locals while retaining `modtm_telemetry`. Pr-check snapshots and restores source files while reporting transform drift. |
