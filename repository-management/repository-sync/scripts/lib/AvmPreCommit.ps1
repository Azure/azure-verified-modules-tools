...
"@
    ...
    try {
        ...
        $null = Invoke-RepositoryGit -Arguments ($clone + @("https://github.com/$Repository.git", $root)) -WorkingDirectory $parent
        if ($AllowedPaths.Count -gt 0) {
        ...
        try {
            ...
            if (-not $context.PullRequest) {
                ...
                $prUrl = Invoke-RepositoryGitHub -Arguments @(
                    'pr', 'create', "--repo=$Repository", "--base=$DefaultBranch", "--head=$branch",
                    '--title', $Title, '--body-file', $bodyFile, '--no-maintainer-edit'
                )
            ...
            Assert-RepositorySyncCandidate -Context $context
            if ($PlanOnly) { $result.Status = 'Planned'; return $result }
            if (-not $repo.allow_squash_merge) { throw [System.InvalidOperationException]::new('Squash merging is unavailable on the synchronization target.') }
            $merge = @(
